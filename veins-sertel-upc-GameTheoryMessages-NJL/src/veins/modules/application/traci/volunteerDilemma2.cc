...
#include "veins/modules/application/traci/volunteerDilemma2.h"
...
const simsignalwrap_t volunteerDilemma2::parkingStateChangedSignal = simsignalwrap_t(TRACI_SIGNAL_PARKING_CHANGE_NAME);
const double timeSlot = 0.000013;

Define_Module(volunteerDilemma2);

void volunteerDilemma2::initialize(int stage) {
/*
...
volunteerDilemma2::volunteerDilemma2()
{
...
double volunteerDilemma2::betaCalculation(int k, int n, double alfa)
{
...
void volunteerDilemma2::onBeacon(WaveShortMessage* wsm) {
...
double volunteerDilemma2::forwardingGame(int N,int M, vector<long double> beta){
       double** mat;
       mat = new double*[N];
       for(int i = 0; i < N; ++i)
           mat[i] = new double[M];
...
double volunteerDilemma2::getLinkQuality(WaveShortMessage* wsm){
...
double volunteerDilemma2::calBeq(double k, double u, double r, double n){

    return pow((k/((u*(1+r*(n-1))))),(1/(n-1)));
}

double volunteerDilemma2::calUtility(double ageFactor, double distanceFactor, double linkQualityFactor)
{
...
double volunteerDilemma2::calAgeFactor(double timeElapsed)
{
...
double volunteerDilemma2::calS(updatedWeightsMetrics vectorR, double r)
{
   double s=0;
   double max=0;
...
double volunteerDilemma2::calAverage(vectorMetrics metric, simtime_t tHello)
{
...
void volunteerDilemma2::onData(WaveShortMessage* wsm) {
...
void volunteerDilemma2::sendMessage(std::string blockedRoadId) {
    sentMessage = true;
...
void volunteerDilemma2::receiveSignal(cComponent* source, simsignal_t signalID, cObject* obj) {
    Enter_Method_Silent();
...
void volunteerDilemma2::handleParkingUpdate(cObject* obj) {
...
void volunteerDilemma2::handlePositionUpdate(cObject* obj) {
...
void volunteerDilemma2::sendWSM(WaveShortMessage* wsm) {
	if (isParking && !sendWhileParking) return;
	sendDelayedDown(wsm,individualOffset);
}

void volunteerDilemma2::handleSelfMsg(cMessage *msg)
{
...
void volunteerDilemma2::forwardMessage(double p)
{
/*
...
bool volunteerDilemma2::hostIsClosestToJunction(string junctionId)
{
...
bool volunteerDilemma2::vehicleOnJunction()
{
...
string volunteerDilemma2::getIdJunction()
{
    // check to see if this host is near an intersection
...
double volunteerDilemma2::getMin(std::map<std::string, double> mymap)
{
...
double volunteerDilemma2::distanceFactorCalc(double d_sr, double d_ri, double r_max, bool  r_vehicleOnIntersection)
{
...
void volunteerDilemma2::printPhLayer(WaveShortMessage* wsm) {
...
void volunteerDilemma2::printMacLayer(WaveShortMessage* wsm) {
...
double volunteerDilemma2::getChannelQuality() {
...
double volunteerDilemma2::getSignalQuality(WaveShortMessage* wsm) {
...
double volunteerDilemma2::getCollisionProbability() {
...
void volunteerDilemma2::printTracyLayer(WaveShortMessage* wsm) {
...
double volunteerDilemma2::calcAbe(WaveShortMessage* wsm)
{
...
double volunteerDilemma2::calcDistJoin(WaveShortMessage* wsm)
{
...
int volunteerDilemma2::forwardElim(double** mat, int N, int M){
...
vector<long double> volunteerDilemma2::backSub(double** mat, int N, int M){
...
vector<long double>  volunteerDilemma2::gaussianElimination(double** mat, int N, int M){
...
void  volunteerDilemma2::swap_row(double** mat, int N, int M, int i, int j){
...
void volunteerDilemma2::print(double** mat, int N, int M){
