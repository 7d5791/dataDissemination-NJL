...
#include "veins/modules/application/traci/AveForwardGameTheory.h"
...
const simsignalwrap_t AveForwardGameTheory::parkingStateChangedSignal = simsignalwrap_t(TRACI_SIGNAL_PARKING_CHANGE_NAME);
const double timeSlot = 0.000013;

Define_Module(AveForwardGameTheory);

void AveForwardGameTheory::initialize(int stage) {
/*
...
AveForwardGameTheory::AveForwardGameTheory()
{
...
double AveForwardGameTheory::betaCalculation(int k, int n, double alfa)
{
...
void AveForwardGameTheory::onBeacon(WaveShortMessage* wsm) {
    receivedBeacons++;
...
////////////////////////////////////////////////////////////////////
  ...
  double s2=calS(r2,r22);
    ...
    //1/3
    wt.push_back((simtime_t)(SLOTLENGTH_11P*timeSlot*0.35*(1-p.back())+SimTime(randomWaitingTime,SIMTIME_US)));
...
///////////////////////////////////////////////////////////////////
//Forwarding Game
   vectorBeta[wsm->getCarId()]=betaCalculation(4,3,(double)(calcAbe(wsm)*0.5+d*0.5));

}

double AveForwardGameTheory::forwardingGame(int N,int M, vector<long double> beta){
       double** mat;
       mat = new double*[N];
       for(int i = 0; i < N; ++i)
           mat[i] = new double[M];
...
double AveForwardGameTheory::getLinkQuality(WaveShortMessage* wsm){
...
double AveForwardGameTheory::calBeq(double k, double u, double r, double n){

    return pow((k/((u*(1+r*(n-1))))),(1/(n-1)));
}


double AveForwardGameTheory::calUtility(double ageFactor, double distanceFactor, double linkQualityFactor)
{
...
double AveForwardGameTheory::calAgeFactor(double timeElapsed)
{
...
double AveForwardGameTheory::calS(updatedWeightsMetrics vectorR, double r)
{
   double s=0;
   double max=0;
...
double AveForwardGameTheory::calAverage(vectorMetrics metric, simtime_t tHello)
{
...
void AveForwardGameTheory::onData(WaveShortMessage* wsm) {
    receivedData++;
...
///////#### Change availability by bandwidth
     ...
     double si=(1-S_i)*betaCalculation(4,3,(double)(calcAbe(wsm)*0.5+d*0.5));
...
void AveForwardGameTheory::sendMessage(std::string blockedRoadId) {
    sentMessage = true;
...
void AveForwardGameTheory::receiveSignal(cComponent* source, simsignal_t signalID, cObject* obj) {
    Enter_Method_Silent();
...
void AveForwardGameTheory::handleParkingUpdate(cObject* obj) {
...
void AveForwardGameTheory::handlePositionUpdate(cObject* obj) {
...
void AveForwardGameTheory::sendWSM(WaveShortMessage* wsm) {
	if (isParking && !sendWhileParking) return;
	sendDelayedDown(wsm,individualOffset);
}

void AveForwardGameTheory::handleSelfMsg(cMessage *msg)
{

    // for "data" and "beacon" self messages
    if ((!strcmp(msg->getName(), "timeoutEvent"))) {
        if(vectorBeta.size()<3) forwardMessage(1);
...
void AveForwardGameTheory::forwardMessage(double p)
{
/*
...
bool AveForwardGameTheory::hostIsClosestToJunction(string junctionId)
{
...
bool AveForwardGameTheory::vehicleOnJunction()
{
...
string AveForwardGameTheory::getIdJunction()
{
    // check to see if this host is near an intersection
...
double AveForwardGameTheory::getMin(std::map<std::string, double> mymap)
{
...
double AveForwardGameTheory::distanceFactorCalc(double d_sr, double d_ri, double r_max, bool  r_vehicleOnIntersection)
{
...
void AveForwardGameTheory::printPhLayer(WaveShortMessage* wsm) {
...
void AveForwardGameTheory::printMacLayer(WaveShortMessage* wsm) {
...
double AveForwardGameTheory::getChannelQuality() {
...
double AveForwardGameTheory::getSignalQuality(WaveShortMessage* wsm) {
...
double AveForwardGameTheory::getCollisionProbability() {
...
void AveForwardGameTheory::printTracyLayer(WaveShortMessage* wsm) {
...
double AveForwardGameTheory::calcAbe(WaveShortMessage* wsm)
{
...
double AveForwardGameTheory::calcDistJoin(WaveShortMessage* wsm)
{
...
int AveForwardGameTheory::forwardElim(double** mat, int N, int M){
...
vector<long double> AveForwardGameTheory::backSub(double** mat, int N, int M){
...
vector<long double>  AveForwardGameTheory::gaussianElimination(double** mat, int N, int M){
...
void  AveForwardGameTheory::swap_row(double** mat, int N, int M, int i, int j){
...
void AveForwardGameTheory::print(double** mat, int N, int M){
