...
#include "veins/modules/application/traci/Flooding.h"


using Veins::TraCIMobilityAccess;
using Veins::AnnotationManagerAccess;

const simsignalwrap_t Flooding::parkingStateChangedSignal = simsignalwrap_t(TRACI_SIGNAL_PARKING_CHANGE_NAME);
...
double Flooding::betaCalculation(int k, int n, double alfa)
{
...
double Flooding::forwardingGame(int N,int M, vector<long double> beta){
       double** mat;
       mat = new double*[N];
       for(int i = 0; i < N; ++i)
           mat[i] = new double[M];
...
double Flooding::getLinkQuality(WaveShortMessage* wsm){
...
double Flooding::calBeq(double k, double u, double r, double n){

    return pow((k/((u*(1+r*(n-1))))),(1/(n-1)));
}


double Flooding::calUtility(double ageFactor, double distanceFactor, double linkQualityFactor)
{
...
double Flooding::calAgeFactor(double timeElapsed)
{
...
double Flooding::calS(updatedWeightsMetrics vectorR, double r)
{
   double s=0;
   double max=0;
...
double Flooding::calAverage(vectorMetrics metric, simtime_t tHello)
{
...
void Flooding::forwardMessage(double p)
{
/*
...
bool Flooding::hostIsClosestToJunction(string junctionId)
{
...
bool Flooding::vehicleOnJunction()
{
...
double Flooding::getMin(std::map<std::string, double> mymap)
{
...
double Flooding::distanceFactorCalc(double d_sr, double d_ri, double r_max, bool  r_vehicleOnIntersection)
{
...
void Flooding::printPhLayer(WaveShortMessage* wsm) {
...
void Flooding::printMacLayer(WaveShortMessage* wsm) {
...
double Flooding::getChannelQuality() {
...
double Flooding::getSignalQuality(WaveShortMessage* wsm) {
...
double Flooding::getCollisionProbability() {
...
void Flooding::printTracyLayer(WaveShortMessage* wsm) {
...
double Flooding::calcAbe(WaveShortMessage* wsm)
{
...
double Flooding::calcDistJoin(WaveShortMessage* wsm)
{
...
int Flooding::forwardElim(double** mat, int N, int M){
...
vector<long double> Flooding::backSub(double** mat, int N, int M){
...
vector<long double>  Flooding::gaussianElimination(double** mat, int N, int M){
...
void  Flooding::swap_row(double** mat, int N, int M, int i, int j){
...
void Flooding::print(double** mat, int N, int M){
