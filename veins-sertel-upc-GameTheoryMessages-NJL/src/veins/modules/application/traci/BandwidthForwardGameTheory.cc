...
#include "veins/modules/application/traci/BandwidthForwardGameTheory.h"
...
const simsignalwrap_t BandwidthForwardGameTheory::parkingStateChangedSignal = simsignalwrap_t(TRACI_SIGNAL_PARKING_CHANGE_NAME);
const double timeSlot = 0.000013;

Define_Module(BandwidthForwardGameTheory);

void BandwidthForwardGameTheory::initialize(int stage) {
/*
...
BandwidthForwardGameTheory::BandwidthForwardGameTheory()
{
...
double BandwidthForwardGameTheory::betaCalculation(int k, int n, double alfa)
{
...
void BandwidthForwardGameTheory::onBeacon(WaveShortMessage* wsm) {
    receivedBeacons++;
...
//Forwarding Game
   ...
   vectorBeta[wsm->getCarId()]=betaCalculation(4,3,(double)calcAbe(wsm));

}

double BandwidthForwardGameTheory::forwardingGame(int N,int M, vector<long double> beta){
       double** mat;
       mat = new double*[N];
       for(int i = 0; i < N; ++i)
           mat[i] = new double[M];
...
double BandwidthForwardGameTheory::getLinkQuality(WaveShortMessage* wsm){
...
double BandwidthForwardGameTheory::calBeq(double k, double u, double r, double n){

    return pow((k/((u*(1+r*(n-1))))),(1/(n-1)));
}


double BandwidthForwardGameTheory::calUtility(double ageFactor, double distanceFactor, double linkQualityFactor)
{
...
double BandwidthForwardGameTheory::calAgeFactor(double timeElapsed)
{
...
double BandwidthForwardGameTheory::calS(updatedWeightsMetrics vectorR, double r)
{
   double s=0;
   double max=0;
...
double BandwidthForwardGameTheory::calAverage(vectorMetrics metric, simtime_t tHello)
{
...
void BandwidthForwardGameTheory::onData(WaveShortMessage* wsm) {
    receivedData++;
...
void BandwidthForwardGameTheory::sendMessage(std::string blockedRoadId) {
    sentMessage = true;
...
void BandwidthForwardGameTheory::receiveSignal(cComponent* source, simsignal_t signalID, cObject* obj) {
    Enter_Method_Silent();
...
void BandwidthForwardGameTheory::handleParkingUpdate(cObject* obj) {
...
void BandwidthForwardGameTheory::handlePositionUpdate(cObject* obj) {
...
void BandwidthForwardGameTheory::sendWSM(WaveShortMessage* wsm) {
	if (isParking && !sendWhileParking) return;
	sendDelayedDown(wsm,individualOffset);
}

void BandwidthForwardGameTheory::handleSelfMsg(cMessage *msg)
{
...
void BandwidthForwardGameTheory::forwardMessage(double p)
{
/*
...
bool BandwidthForwardGameTheory::hostIsClosestToJunction(string junctionId)
{
...
bool BandwidthForwardGameTheory::vehicleOnJunction()
{
...
string BandwidthForwardGameTheory::getIdJunction()
{
    // check to see if this host is near an intersection
...
double BandwidthForwardGameTheory::getMin(std::map<std::string, double> mymap)
{
...
double BandwidthForwardGameTheory::distanceFactorCalc(double d_sr, double d_ri, double r_max, bool  r_vehicleOnIntersection)
{
...
void BandwidthForwardGameTheory::printPhLayer(WaveShortMessage* wsm) {
...
void BandwidthForwardGameTheory::printMacLayer(WaveShortMessage* wsm) {
...
double BandwidthForwardGameTheory::getChannelQuality() {
...
double BandwidthForwardGameTheory::getSignalQuality(WaveShortMessage* wsm) {
...
double BandwidthForwardGameTheory::getCollisionProbability() {
    ...
    cp=0;
    (tw.dbl()-listTimeWindow.back().dbl())>0 ? cp=(co.dbl()-listChannelOccupancy.back().dbl())/(tw.dbl()-listTimeWindow.back().dbl()):cp=0;
...
void BandwidthForwardGameTheory::printTracyLayer(WaveShortMessage* wsm) {
...
double BandwidthForwardGameTheory::calcAbe(WaveShortMessage* wsm)
{
...
double BandwidthForwardGameTheory::calcDistJoin(WaveShortMessage* wsm)
{
...
int BandwidthForwardGameTheory::forwardElim(double** mat, int N, int M){
...
vector<long double> BandwidthForwardGameTheory::backSub(double** mat, int N, int M){
...
vector<long double>  BandwidthForwardGameTheory::gaussianElimination(double** mat, int N, int M){
...
void  BandwidthForwardGameTheory::swap_row(double** mat, int N, int M, int i, int j){
...
void BandwidthForwardGameTheory::print(double** mat, int N, int M){
