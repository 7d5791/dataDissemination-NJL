...
#include "veins/modules/application/traci/volunteerDilemma.h"


using Veins::TraCIMobilityAccess;
using Veins::AnnotationManagerAccess;

const simsignalwrap_t volunteerDilemma::parkingStateChangedSignal = simsignalwrap_t(TRACI_SIGNAL_PARKING_CHANGE_NAME);
const double timeSlot = 0.000013;

Define_Module(volunteerDilemma);

void volunteerDilemma::initialize(int stage) {
...
volunteerDilemma::volunteerDilemma()
{
...
double volunteerDilemma::betaCalculation(int k, int n, double alfa)
{
...
void volunteerDilemma::onBeacon(WaveShortMessage* wsm) {
    receivedBeacons++;
...
////////////////////////////////////////////////////////////////////
  ...
  // EV<<"calUtility: "<<calUtility(calAgeFactor(simTime().dbl()-wsm->getCreationTime().dbl()),d,lq)<<endl;
   ...
   EV<<"calBeqAsimetrico: r=1 :"<<calBeqAsimetrico(1,(double)calUtility(calAgeFactor(simTime().dbl()-wsm->getCreationTime().dbl()),d,lq),1,(double)ListBeacon.CounterBeacons())<<endl;

   EV<<"calBeqAsimetrico: r=0 :"<<calBeqAsimetrico(1,(double)calUtility(calAgeFactor(simTime().dbl()-wsm->getCreationTime().dbl()),d,lq),0,(double)ListBeacon.CounterBeacons())<<endl;

    //EV<<"calBeqAsimetrico: r=0 :"<<calBeqAsimetrico(1,0.5,0,(double)ListBeacon.CounterBeacons())<<endl;

///////////////////////////////////////////////////////////////////
//Volunteer Dilemma Asimetric
   double kj=1;
   vectorUtility[wsm->getCarId()]=kj/(double)calUtility(calAgeFactor(simTime().dbl()-wsm->getCreationTime().dbl()),d,lq);

}


double volunteerDilemma::forwardingGame(int N,int M, vector<long double> beta){
       double** mat;
       mat = new double*[N];
       for(int i = 0; i < N; ++i)
           mat[i] = new double[M];
...
double volunteerDilemma::vodM(vector<long double> u){
...
double volunteerDilemma::calBeqAsimetricoMult(double ki, double ui, double kjuj, double n)
{
...
double volunteerDilemma::getLinkQuality(WaveShortMessage* wsm){
...
double volunteerDilemma::calBeqAsimetrico(double k, double u, double r, double n){

    return pow((k/((u*(1+r*(n-1))))),(1/(n-1)));
}


double volunteerDilemma::calUtility(double ageFactor, double distanceFactor, double linkQualityFactor)
{
...
double volunteerDilemma::calAgeFactor(double timeElapsed)
{
...
double volunteerDilemma::calS(updatedWeightsMetrics vectorR, double r)
{
   double s=0;
   double max=0;
...
double volunteerDilemma::calAverage(vectorMetrics metric, simtime_t tHello)
{
...
void volunteerDilemma::onData(WaveShortMessage* wsm) {
    receivedData++;
    ...
    EV<<"calBeqAsimetrico: r=1 :"<<calBeqAsimetrico(1,(double)calUtility(calAgeFactor(simTime().dbl()-wsm->getCreationTime().dbl()),d,lq),1,(double)ListBeacon.CounterBeacons())<<endl;
     ...
     for( map<long,long double >::iterator it = vectorUtility.begin(); it != vectorUtility.end(); ++it ) {
     ...
     double Bi=calBeqAsimetricoMult(ki,(double)calUtility(calAgeFactor(simTime().dbl()-wsm->getCreationTime().dbl()),d,lq),B_i,N);

     ProbabilityThreshold=1-Bi;
...
void volunteerDilemma::sendMessage(std::string blockedRoadId) {
    sentMessage = true;
...
void volunteerDilemma::receiveSignal(cComponent* source, simsignal_t signalID, cObject* obj) {
    Enter_Method_Silent();
...
void volunteerDilemma::handleParkingUpdate(cObject* obj) {
...
void volunteerDilemma::handlePositionUpdate(cObject* obj) {
...
void volunteerDilemma::sendWSM(WaveShortMessage* wsm) {
	if (isParking && !sendWhileParking) return;
	sendDelayedDown(wsm,individualOffset);
}

void volunteerDilemma::handleSelfMsg(cMessage *msg)
{
...
void volunteerDilemma::forwardMessage(double p)
{
/*
...
bool volunteerDilemma::hostIsClosestToJunction(string junctionId)
{
...
bool volunteerDilemma::vehicleOnJunction()
{
...
string volunteerDilemma::getIdJunction()
{
    // check to see if this host is near an intersection
...
double volunteerDilemma::getMin(std::map<std::string, double> mymap)
{
...
double volunteerDilemma::distanceFactorCalc(double d_sr, double d_ri, double r_max, bool  r_vehicleOnIntersection)
{
...
void volunteerDilemma::printPhLayer(WaveShortMessage* wsm) {
...
void volunteerDilemma::printMacLayer(WaveShortMessage* wsm) {
...
double volunteerDilemma::getChannelQuality() {
...
double volunteerDilemma::getSignalQuality(WaveShortMessage* wsm) {
...
double volunteerDilemma::getCollisionProbability() {
...
void volunteerDilemma::printTracyLayer(WaveShortMessage* wsm) {
...
double volunteerDilemma::calcAbe(WaveShortMessage* wsm)
{
...
double volunteerDilemma::calcDistJoin(WaveShortMessage* wsm)
{
...
int volunteerDilemma::forwardElim(double** mat, int N, int M){
...
vector<long double> volunteerDilemma::backSub(double** mat, int N, int M){
...
vector<long double>  volunteerDilemma::gaussianElimination(double** mat, int N, int M){
...
void  volunteerDilemma::swap_row(double** mat, int N, int M, int i, int j){
...
void volunteerDilemma::print(double** mat, int N, int M){
