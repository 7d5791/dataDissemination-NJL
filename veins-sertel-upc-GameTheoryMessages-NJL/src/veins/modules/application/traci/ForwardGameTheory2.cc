...
#include "veins/modules/application/traci/ForwardGameTheory2.h"


using Veins::TraCIMobilityAccess;
using Veins::AnnotationManagerAccess;

const simsignalwrap_t ForwardGameTheory2::parkingStateChangedSignal = simsignalwrap_t(TRACI_SIGNAL_PARKING_CHANGE_NAME);
const double timeSlot = 0.000013;

Define_Module(ForwardGameTheory2);

void ForwardGameTheory2::initialize(int stage) {
...
ForwardGameTheory2::ForwardGameTheory2()
{
...
double ForwardGameTheory2::betaCalculation(int k, int n, double alfa)
{
...
void ForwardGameTheory2::onBeacon(WaveShortMessage* wsm) {
    receivedBeacons++;
...
double ForwardGameTheory2::forwardingGame(int N,int M, vector<long double> beta){
       double** mat;
       mat = new double*[N];
       for(int i = 0; i < N; ++i)
           mat[i] = new double[M];
...
double ForwardGameTheory2::getLinkQuality(WaveShortMessage* wsm){
...
double ForwardGameTheory2::calBeq(double k, double u, double r, double n){

    return pow((k/((u*(1+r*(n-1))))),(1/(n-1)));
}


double ForwardGameTheory2::calUtility(double ageFactor, double distanceFactor, double linkQualityFactor)
{
...
double ForwardGameTheory2::calAgeFactor(double timeElapsed)
{
...
double ForwardGameTheory2::calS(updatedWeightsMetrics vectorR, double r)
{
   double s=0;
   double max=0;
...
double ForwardGameTheory2::calAverage(vectorMetrics metric, simtime_t tHello)
{
...
void ForwardGameTheory2::onData(WaveShortMessage* wsm) {
    receivedData++;
...
void ForwardGameTheory2::sendMessage(std::string blockedRoadId) {
    sentMessage = true;
...
void ForwardGameTheory2::receiveSignal(cComponent* source, simsignal_t signalID, cObject* obj) {
    Enter_Method_Silent();
...
void ForwardGameTheory2::handleParkingUpdate(cObject* obj) {
...
void ForwardGameTheory2::handlePositionUpdate(cObject* obj) {
...
void ForwardGameTheory2::sendWSM(WaveShortMessage* wsm) {
	if (isParking && !sendWhileParking) return;
	sendDelayedDown(wsm,individualOffset);
}

void ForwardGameTheory2::handleSelfMsg(cMessage *msg)
{
...
void ForwardGameTheory2::forwardMessage(double p)
{
/*
...
bool ForwardGameTheory2::hostIsClosestToJunction(string junctionId)
{
...
bool ForwardGameTheory2::vehicleOnJunction()
{
...
string ForwardGameTheory2::getIdJunction()
{
    // check to see if this host is near an intersection
...
double ForwardGameTheory2::getMin(std::map<std::string, double> mymap)
{
...
double ForwardGameTheory2::distanceFactorCalc(double d_sr, double d_ri, double r_max, bool  r_vehicleOnIntersection)
{
...
void ForwardGameTheory2::printPhLayer(WaveShortMessage* wsm) {
...
void ForwardGameTheory2::printMacLayer(WaveShortMessage* wsm) {
...
double ForwardGameTheory2::getChannelQuality() {
...
double ForwardGameTheory2::getSignalQuality(WaveShortMessage* wsm) {
...
double ForwardGameTheory2::getCollisionProbability() {
...
void ForwardGameTheory2::printTracyLayer(WaveShortMessage* wsm) {
...
double ForwardGameTheory2::calcAbe(WaveShortMessage* wsm)
{
...
double ForwardGameTheory2::calcDistJoin(WaveShortMessage* wsm)
{
...
int ForwardGameTheory2::forwardElim(double** mat, int N, int M){
...
vector<long double> ForwardGameTheory2::backSub(double** mat, int N, int M){
...
vector<long double>  ForwardGameTheory2::gaussianElimination(double** mat, int N, int M){
...
void  ForwardGameTheory2::swap_row(double** mat, int N, int M, int i, int j){
...
void ForwardGameTheory2::print(double** mat, int N, int M){
